...
- Both nftables locks block outbound ICMP echo-request on non-tunnel interfaces. Outside-tunnel pings need per-IP allowlist in BOTH active locks (2026-03-05)
...
- Background pinger: 1 ping per server per cycle, EWMA α=0.3 smooths across cycles. Persisted to `/var/lib/airvpn-rs/latency.json`. Matches Eddie's 1-ping approach but parallel + while connected (2026-03-07)
